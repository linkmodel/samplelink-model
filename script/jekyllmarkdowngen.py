...
class JekyllMarkdownGenerator(MarkdownGenerator):
    ...
    def visit_type(self, typ: TypeDefinition) -> None:
        ...
        with open(self.dir_path(typ), 'w') as typefile:
            with redirect_stdout(typefile):
                ...
                if type_curie.startswith('https://w3id.org/samplelink/vocab/linkml:types/'):
                    ref = type_curie.split('/')[-1]
                    type_uri = f"https://linkml.github.io/linkml-model/docs/{ref}"
                    type_curie = f"metatype:{ref}"
                elif type_uri.startswith('https://w3id.org/samplelink/vocab/'):
                    ref = type_curie.split('/')[-1]
                    type_uri = f"https://w3id.org/samplelink/vocab/types/{ref}"
    ...
    def element_header(self, obj: Element, name: str, curie: str, uri: str) -> None:
        ...
        if curie.startswith('http'):
            if curie.startswith('https://w3id.org/samplelink/vocab/linkml:types/'):
                simple_name = curie.split('/')[-1]
                uri = f"https://linkml.github.io/linkml-model/docs/{simple_name}"
